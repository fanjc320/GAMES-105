...
class CharacterController():
    ...
    def update_state(self,
        ...
        if self.motion_state == "move":
            ...
            diff_root_pos[:, 1] = 0
            diff_root_rot = (R.from_quat(desired_rot_list[0:6]) * R.from_quat(
                current_motion.joint_rotation[key_frame, 0, :]).inv()).as_rotvec()
            ...
            for i in range(self.cur_frame, self.cur_frame + self.motions[motion_id].motion_length // 2):
                ...
                off_pos, _ = decay_spring_implicit_damping_pos(diff_root_pos[index], diff_root_vel[index], half_time,
                                                               dt / 60)
                off_rot, _ = decay_spring_implicit_damping_rot(diff_root_rot[index], diff_root_avel[index], half_time,
                                                               dt / 60)
                ...
                current_motion.joint_rotation[i % self.motions[motion_id].motion_length, 0, :] = (
                        ...
                        current_motion.joint_rotation[i % self.motions[motion_id].motion_length, 0, :])).as_quat()
            ...
            if self.motion_state != last_motion_state:
                facing_axis = R.from_quat(self.blending_motion.joint_rotation[self.cur_frame, 0, :]).apply(
                    np.array([0, 0, 1])).flatten()[[0, 2]]
                current_motion = current_motion.translation_and_rotation(0, self.blending_motion.joint_position[
...
# https://github.com/DIOYF/GamesPersonalHW/blob/main/games105/Lab2_task2.py